...
def gpsRead(mqtt_connection, topic, pub_period, debug):
    ...
    for line in trip:
        ...
        try:
            ...
            if line.startswith('$GPGGA'):
                ...
                # Publish to AWS IoT
                jsonPayload(data, mqtt_connection, topic, debug)
...
def jsonPayload(gps_data, mqtt_connection, topic, debug):
    ...
    try:
        ...
        # Debug information
        if debug:
            print(f"Published data to {topic}: {json_data}")
...
def mqtt_send_data():
    ...
    gpsRead(mqtt_connection, topic, pub_period, debug)
